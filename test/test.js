...
describe("starting tests", function () {
    ...
    before(async () =>{
      ...
      contract = new web3.eth.Contract(abi_contract, "0xC13eac3B4F9EED480045113B7af00F7B5655Ece8")
    ...
    describe("tests", function(){
        ...
        it("testing permit", async function(){
            ...
            var c = await web3.eth.accounts.sign(hash1_for_encodePacked, private_key);
            //console.log(c);
            const {signature, messageHash, v, r, s} = c;

            const d = await web3.eth.accounts.recover({messageHash: messageHash, v: v, r: r, s: s});
            console.log(d);
            ...
            const check=await instance.permit(PERMIT_TYPEHASH,DOMAIN_SEPARATOR,currentValidNonce,spender.address,sender.address,10000000,1000000000000,c.v,c.r,c.s);
            console.log(check);

            console.log("should be:", spender.address);
            //await aave_token_contract.connect(sender).permit(spender.address,sender.address,10000000,1000000000000,c.v,c.s,c.r);
        });
